...
let engravingCheck = Array.prototype.slice.call(
  document.querySelectorAll(".engraving")
);
...
function addRow(e) {
  ...
  newRow.innerHTML = `
                <th><input type="checkbox" name="${
                  productRow.children[0].firstElementChild.name
                }" class="selection"></th>
...
function adjustPrice(event) {
  ...
  let priceElement =
    event.target.parentElement.parentElement.parentElement.children[6];
...
function parseSelectedItems(selectedItems) {
  ...
  function extractColorSelection(product) {
    for (let i = 0; i < product.children[3].children.length; i += 1) {
      if (product.children[3].children[i].children[0].checked)
        // if color is checked
...
function generateEmail(productArray) {
  ...
  const installmentElements = Array.prototype.slice.call(
    document.querySelector(".installments").children
  );
  ...
  // assigns value of installment selected to order
  installmentElements.forEach(element => {
    if (element.checked) {
      order.installments = Number(element.id);
    }
  });
  ...
  const header = `<h3 id="quoteHeading">Quote #1 - ${
    order.itemsOrdered
  } Gifts w/ ${
    order.discountItems
  } additional at Quantity Discount - <span id="savingsHeader">SAVINGS: $${order.savings.toFixed(
    2
  )}</span></h3>`;
  quote += header;
  ...
  order.products.forEach(product => {
    let productEntry = "";
    if (product.discount) {
      productEntry = `<p class="discountQuote">${
        product.quantity
      } - ${product.color.toUpperCase()} ${
        product.name
      } @ Quantity Discount <span class="productTotal">$${(
        product.quantity * product.fullPrice
      ).toFixed(2)}</span></p>`;
    } else if (product.free) {
      productEntry = `<p class="freeQuote">FREE ${
        product.quantity
      } - ${product.color.toUpperCase()} ${
        product.name
      } for your home! <span class="productTotal">$${(
        product.quantity * product.fullPrice
      ).toFixed(2)}</span></p>`;
    } else {
      productEntry = `<p>${product.quantity} - ${product.color.toUpperCase()} ${
        product.name
  ...
  if (order.giftWrap) {
    ...
    quote += `<p>${order.totalItems -
      order.freeItems} - Gift Wraps<span class="productTotal">$${5 *
      (order.totalItems - order.freeItems)}</span></p>`;
  }

  quote += `<p>FLAT RATE SHIPPING<span class="productTotal">$${
    order.shipping
  }</span></p>`;
  quote += `<p>The Total is $${(order.total + order.shipping).toFixed(
    2
  )} + local tax paid over ${
    order.installments
  } months interest free for a monthly payment of $${(
    (order.total + order.shipping) /
    order.installments
  ).toFixed(
    2
  )} + local tax. With this quote your cost per gift would be <span id="costPerGift">$${Math.ceil(
    order.total / (order.totalItems - order.freeItems)
  )}</span>!</p>`;
  quote += "</div>";
...
function calculateSavings(orders) {
  ...
  discountedItems.forEach(product => {
    console.log(
      `original price: ${product.originalPrice} - fullprice: ${
        product.fullPrice
      }`
    );
